...
def extract_unsubscribe_url(input_string):
    ...
    for i, url in enumerate(urls):
        if 'Unsubscribe' in input_string[input_string.find(url) - 50: input_string.find(url)]:
            return url
...
def main():
    ...
    while sync_from_gmail_api_chx.lower() not in ['y', 'n']:
        sync_from_gmail_api_chx = input('Sync emails from gmail api (y/n) (n) ? \n > ') or 'n'
    ...
    target_sender_email_chx = input('Enter email to unsub from : (news@email-blacks.co.uk) \n > ') or 'news@email-blacks.co.uk'
    valid_email = None
    while valid_email is None:
        latest_email_id = get_latest_email_id(df = df, sender_email=target_sender_email_chx)
    ...
    print(f'Fetching email body for message id : {target_email_id} .. ', end='', flush=True)
